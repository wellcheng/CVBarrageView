...
@interface CVLBarrageRenderView ()
{
    CGFloat _timerInterval;
    CGFloat _duration;
}
@property(nonatomic, assign, readwrite) BOOL isReadyToRender;
@property(nonatomic, assign, readwrite) CVLPlayBarrageViewStatus userRenderStatus;
@property(nonatomic, assign) CGFloat lastRenderTime;
@property (nonatomic, strong) NSArray<CVLBarrageDescriptor *> *originBarrageDescs;   // 原始未经处理的弹幕数据
@property (nonatomic, strong) NSArray<CVLBarrageDescriptor *> *descNeedsRender;     // 已经处理好的弹幕数据
@property (nonatomic,strong) CVLCountdownTrigger *sendBarrageTrigger;
@property (nonatomic, strong) NSArray<NSNumber *> *trajectories;
@end

@implementation CVLBarrageRenderView
...
- (void)propertyInit {
    ...
    _isReadyToRender = NO;
    _userRenderStatus = CVLPlayBarrageViewNotSupport;
}
...
- (void)configTimer {
    _sendBarrageTrigger = [[CVLCountdownTrigger alloc] initWithTarget:self selector:@selector(timeToRenderBarrage:) tickTimes:_timerInterval tickUnit:CVLTimeSecond repeats:YES];
}
...
- (void)setRenderStatus:(CVLPlayBarrageViewStatus)status {
    if (status == CVLPlayBarrageViewOpen) {
        self.userRenderStatus = CVLPlayBarrageViewOpen;
    } else {
        self.userRenderStatus = CVLPlayBarrageViewClose;
        [self clearAllCells];
    }
}
...
- (void)showBarrage:(NSArray<CVLBarrageDescriptor *> *)barrageDescs {
    if (!barrageDescs || barrageDescs.count == 0 || self.userRenderStatus != CVLPlayBarrageViewOpen) {
        return;
    }
    // 获取当前的弹幕分布
    ...
    for (CVLBarrageDescriptor *desc in barrageDescs) {
        // 来一条 渲染一条
        CVLBarrageCell *cell = [[CVLBarrageCell alloc] initWithSpirit:[[CVLBarrageSpirit alloc] initWithDesc:desc]
...
//    NSLog(@"cv getCurrentCellInfo start **");
    ...
    for (UIView *view in [[self.subviews reverseObjectEnumerator] allObjects]) {
        CVLBarrageCell *animCell = nil;
        if ([view isKindOfClass:[CVLBarrageCell class]]) {
            animCell = (CVLBarrageCell *)view;
        }
        if (!animCell ) {
            continue;
        }
 ...
 */
- (void)dispatchNewBarrageCell:(CVLBarrageCell *)cell
...
- (void)prepareForBarrageDescs {
    NSArray<CVLBarrageDescriptor *> *barrageDescs = self.originBarrageDescs;
    // 平均分配弹幕的渲染时间
    ...
    NSArray *barragesWithTime = [CVLBarrageDescriptor assignShootTimeForBarrages:[barragesNeedsTime copy] withDuration:duration];
    [barragesHaveTime addObjectsFromArray:barragesWithTime];
    [barragesHaveTime sortUsingComparator:^NSComparisonResult(CVLBarrageDescriptor * _Nonnull obj1, CVLBarrageDescriptor * _Nonnull obj2) {
...
- (BOOL)ajustTrajectory:(CVLBarrageCell *)cell withCellLayoutInfo:(NSArray *)cellLayoutInfo {
    ...
    if (idleTrajectory == NSNotFound) {
        for (NSUInteger trajectory = 0; trajectory < cellLayoutInfo.count; ++ trajectory) {
            ...
            CVLBarrageCell *rightCell = [cellInfo objectForKey:@"cell"];
...
{
    if (self.userRenderStatus != CVLPlayBarrageViewOpen) {
        // 用户弹幕开发被关闭
        return;
    }
...
- (void)animationCell:(CVLBarrageCell *)cell {
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
...
- (void)pauseCellAnimation {
    for (CVLBarrageCell *cell in self.subviews) {
        [[NSOperationQueue mainQueue] addOperationWithBlock:^{
