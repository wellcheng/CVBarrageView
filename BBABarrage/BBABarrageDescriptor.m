...
@interface BBABarrageDescriptor ()
@property(nonatomic, strong, readwrite) NSDictionary *attributeDict;
@end

@implementation BBABarrageDescriptor
...
- (void)propertyInit {
    ...
    _content = [_attributeDict bba_stringValueForKey:@"content"] ?: @"";
    _shootTime = [[_attributeDict bba_numberValueForKey:@"time"] floatValue];
}
...
+ (NSArray<BBABarrageDescriptor *>*)createByDicts:(NSArray<NSDictionary *> *)dicts {
    if (!dicts || dicts.count == 0) {
        return nil;
    }
    NSMutableArray *descs = [NSMutableArray array];
    for (NSDictionary *dict in dicts) {
        BBABarrageDescriptor *desc = [[BBABarrageDescriptor alloc] initWithAttributeDict:dict];
        if (desc) {
...
/**
 给没有时间的弹幕分配时间
 */
+ (NSArray<BBABarrageDescriptor *> *)assignShootTimeForBarrages:(NSArray<BBABarrageDescriptor *> *)barrageDescs
